...
export default function App() {
  ...
  // Fetch settings from Full-Stack SQL Backend
  useEffect(() => {
    let active = true;
    (async () => {
      try {
        const [resBkash, resDelivery, resChat] = await Promise.all([
          fetch('/api/settings/bkash_merchant'),
          fetch('/api/settings/delivery_service'),
          fetch('/api/settings/chat_support')
        ]);
        if (active && resBkash.ok) setBkashSettings(await resBkash.json());
        if (active && resDelivery.ok) setDeliverySettings(await resDelivery.json());
        if (active && resChat.ok) setChatSupportSettings(await resChat.json());
      } catch (e) {
        console.error('Failed to load settings from SQL database backend:', e);
      }
    })();
    return () => { active = false; };
  }, []);
  ...
  const handleSaveBkashSettings = async (settings: BkashSettings) => {
    try {
      setBkashSettings(settings);
      await fetch('/api/settings/bkash_merchant', {
        method: 'PUT',
  ...
  const handleSaveDeliverySettings = async (settings: DeliverySettings) => {
    try {
      ...
      await fetch('/api/settings/delivery_service', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(settings)
      });
  ...
  const handleSaveChatSupportSettings = async (settings: ChatSupportSettings) => {
    try {
      setChatSupportSettings(settings);
      await fetch('/api/settings/chat_support', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(settings)
      });
  ...
  // SQL DB States & Admin Mode State
  ...
  useEffect(() => {
    ...
    const fetchProductsAndOrders = async () => {
      ...
      try {
        ...
        if (revRes.ok && active) {
          const revItems = await revRes.json();
          if (Array.isArray(revItems) && revItems.length > 0) {
            setReviews(revItems);
          }
        }
      } catch (err) {
        console.warn('[Full-Stack Sync] API poll failed, using local fallback:', err);
        if (active) setApiError('offline');
      }
    };
  ...
  const handleAddNewReview = (comment: string, rating: number, userName: string) => {
    ...
    // Save review to Full-Stack SQL Database
    fetch('/api/reviews', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(newRev)
    }).catch(err => console.error('Failed to save review to SQL database:', err));
  ...
  const deliveryFee = cartSubtotal > 0 ? (cartSubtotal >= 1000 ? 0 : 80) : 0;
  ...
  const appContent = (
    <div className={`flex flex-col ${(isMobileView && windowWidth >= 768) ? 'min-h-full' : 'min-h-screen'} bg-linear-to-b from-slate-50 to-slate-100 text-slate-800 dark:from-slate-950 dark:to-slate-900 dark:text-slate-100`} id="main-application-view">
      ...
      <div className="bg-emerald-600 py-1.5 px-4 text-center text-xs font-semibold text-white flex justify-between items-center sm:px-8 dark:bg-emerald-800">
        <span className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-3 mx-auto text-[11px] sm:text-xs">
          <span className="flex items-center gap-1.5">
            <Bike className="h-4 w-4 animate-bounce" />
            {dict.tagline}
          </span>
          <span className="hidden sm:inline opacity-60">|</span>
          <span className="bg-amber-400 text-slate-950 px-2 py-0.5 rounded-full font-black text-[10px] uppercase tracking-wide flex items-center gap-1 shadow-xs">
            🎁 {lang === 'en' ? 'FREE Delivery on ৳1000+' : '৳১০০০+ অর্ডারে ফ্রি ডেলিভারি (৳৮০ ছাড়)'}
          </span>
        </span>
      </div>
      ...
      {apiError && (
        <div className="bg-amber-500/10 dark:bg-amber-950/20 border-b border-amber-500/30 text-amber-800 dark:text-amber-300 py-2 px-4 text-xs font-medium text-center">
          <span className="inline-flex items-center gap-1.5">
            ⚠️ {lang === 'en' 
              ? 'SQL Database Offline: Local fallback is active. Data is being stored in browser session.' 
              : 'SQL ডাটাবেস কানেকশন বিচ্ছিন্ন: লোকাল ব্যাকআপ সক্রিয় আছে। ডাটা ব্রাউজারে সেভ হচ্ছে।'}
          </span>
        </div>
      )}
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-xs border-b border-slate-100 dark:bg-slate-900/90 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 py-3 sm:px-6 lg:px-8 flex flex-wrap lg:flex-nowrap items-center justify-between gap-3 lg:gap-4">
          ...
          <div className="flex items-center gap-4 order-1">
            ...
            >
              ...
              <div className="flex h-9 w-9 sm:h-11 sm:w-11 items-center justify-center rounded-xl sm:rounded-2xl bg-[#0B1E28] text-white shadow-lg border border-slate-100/10 transform group-hover:scale-105 group-hover:rotate-6 transition-all duration-300">
                <svg viewBox="0 0 100 100" className="h-6 w-6 sm:h-7 sm:w-7 text-[#FF8A00] fill-none" stroke="currentColor" strokeWidth="9.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              <div className="leading-tight">
                <h1 className="text-lg sm:text-xl font-black tracking-tight flex items-center flex-wrap sm:flex-nowrap">
                  {lang === 'en' ? (
                    <>
                  ...
                  <span className="hidden sm:inline-block ml-2 text-[9px] font-black bg-emerald-500 text-white rounded-md px-1.5 py-0.5 animate-pulse tracking-normal shrink-0">{dict.deliveryTime}</span>
                </h1>
                <p className="text-[9px] sm:text-[10px] text-zinc-400 font-bold tracking-widest">{lang === 'en' ? 'FAST COMMERCE' : 'দ্রুতগতির সুপারশপ'}</p>
              </div>
            </div>
          ...
          <div className="flex-1 w-full lg:w-auto max-w-none lg:max-w-xl relative order-3 lg:order-2 mt-1 sm:mt-0">
            ...
            <input
              ...
              className="w-full text-sm rounded-xl sm:rounded-2xl border border-slate-200 bg-slate-50 pl-11 pr-4 py-2.5 sm:py-3 text-slate-900 placeholder-slate-400 outline-hidden transition-all focus:border-emerald-500 focus:bg-white dark:border-slate-800 dark:bg-slate-950 dark:text-white dark:focus:border-emerald-500"
          ...
          <div className="flex items-center justify-end gap-2 sm:gap-3.5 order-2 lg:order-3">
            ...
            <button
              ...
              className={`flex items-center justify-center gap-1.5 font-extrabold text-xs border rounded-xl w-9 h-9 sm:w-auto sm:h-auto sm:px-2.5 sm:py-2 transition-all cursor-pointer ${
                isAdminMode 
            ...
            >
              {isAdminMode ? <LogOut className="h-4 w-4 sm:h-4 sm:w-4" /> : <Lock className="h-4 w-4 sm:h-4 sm:w-4" />}
            ...
            <button
              ...
              className="flex items-center justify-center gap-1 hover:text-emerald-500 font-semibold text-xs border border-slate-100 dark:border-slate-800 rounded-xl w-9 h-9 sm:w-auto sm:h-auto sm:px-2.5 sm:py-2 hover:bg-slate-50 dark:hover:bg-slate-950 transition-all"
            ...
            <button
              ...
              className="flex items-center justify-center border border-slate-100 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-950 text-slate-500 dark:text-slate-400 transition-all w-9 h-9 sm:p-2 sm:w-auto sm:h-auto"
            ...
            <button
              ...
              className="relative flex items-center justify-center gap-2 rounded-xl sm:rounded-2xl bg-emerald-600 w-9 h-9 sm:w-auto sm:h-auto sm:px-4 sm:py-2.5 text-white shadow-md hover:bg-emerald-700 transition-colors"
            ...
            >
              ...
              <div className="hidden sm:block text-left text-xs">
                {totalCartQty > 0 ? (
                  ...
                  <span className="text-xs font-bold font-heading block pt-0.5">{lang === 'en' ? 'Cart' : 'কার্ট'}</span>
                )}
              </div>

              {/* Float count badge */}
              {totalCartQty > 0 && (
                <span className="absolute -top-1.5 -right-1.5 flex h-4 w-4 sm:h-5 sm:w-5 items-center justify-center rounded-full bg-yellow-400 text-[9px] sm:text-[10px] font-black text-slate-950 leading-none shadow-xs animate-pulse">
                  {totalCartQty}
                </span>
              )}
            </button>
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-950/40">
                  {cart.length > 0 && (
                    <div className="space-y-3">
                      ...
                      <div className="bg-white dark:bg-slate-900 border border-emerald-500/20 dark:border-emerald-500/30 rounded-2xl p-3.5 shadow-xs">
                        <div className="flex items-center justify-between text-xs font-black mb-1.5">
                          <span className="flex items-center gap-1.5 text-slate-800 dark:text-slate-200">
                            <Truck className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                            {cartSubtotal >= 1000 
                              ? (lang === 'en' ? '🎉 You unlocked FREE Delivery!' : '🎉 আপনি ফ্রি ডেলিভারি আনলক করেছেন!')
                              : (lang === 'en' 
                                  ? `Add ৳${1000 - cartSubtotal} more for FREE Delivery` 
                                  : `আর ৳${1000 - cartSubtotal} যোগ করলেই ফ্রি ডেলিভারি`)}
                          </span>
                          <span className="text-emerald-600 dark:text-emerald-400 font-extrabold text-[11px]">
                            {cartSubtotal >= 1000 ? (lang === 'en' ? '৳80 Saved' : '৳৮০ মওকুফ') : `${Math.min(100, Math.round((cartSubtotal / 1000) * 100))}%`}
                          </span>
                        </div>
                        {/* Progress Bar */}
                        <div className="w-full bg-slate-100 dark:bg-slate-800 h-2 rounded-full overflow-hidden">
                          <div 
                            ...
                            style={{ width: `${Math.min(100, Math.round((cartSubtotal / 1000) * 100))}%` }}
                          />
                        </div>
                      </div>
      ...
      {chatSupportSettings && chatSupportSettings.activePlatform !== 'none' && (
        ...
        >
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-extrabold text-[11px] px-3.5 py-2 rounded-2xl shadow-lg max-w-[200px] text-center animate-bounce mb-1">
            {lang === 'en' ? '👋 Chat with us!' : '👋 আমাদের সাথে চ্যাট করুন!'}
          </div>

          <div className="flex flex-row gap-3">
            {(chatSupportSettings.activePlatform === 'facebook' || chatSupportSettings.activePlatform === 'both') && chatSupportSettings.facebookUrl && (
              <a
                href={
                ...
                className="flex items-center justify-center w-10 h-10 rounded-full text-white shadow-2xl hover:scale-110 active:scale-95 transition-all bg-[#1877F2]"
                title="Messenger"
              >
            ...
            {(chatSupportSettings.activePlatform === 'whatsapp' || chatSupportSettings.activePlatform === 'both') && chatSupportSettings.whatsappNumber && (
              <a
                href={`https://wa.me/${chatSupportSettings.whatsappNumber.replace(/[^0-9]/g, '')}?text=${encodeURIComponent(chatSupportSettings.whatsappMessage || '')}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-10 h-10 rounded-full text-white shadow-2xl hover:scale-110 active:scale-95 transition-all bg-[#25D366]"
                title="WhatsApp"
              >
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24">
      ...
      {/* Footer copyright */}
      <footer className="bg-white border-t border-slate-100 py-2.5 text-center text-xs text-slate-400 dark:bg-slate-950 dark:border-slate-850">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center justify-center gap-1.5">
          <div className="flex flex-col items-center justify-center">
            <p className="font-extrabold text-sm text-slate-700 dark:text-slate-300">Master Mart®</p>
            <p className="text-slate-500 dark:text-slate-400 font-medium text-[10px] sm:text-xs">An Online Grocery Brand by Bismillah Traders</p>
          </div>
            ...
            <div className="flex items-center justify-center gap-4">
              {(chatSupportSettings.activePlatform === 'whatsapp' || chatSupportSettings.activePlatform === 'both') && chatSupportSettings.whatsappNumber && (
                <a 
                  ...
                  target="_blank" 
                  rel="noopener noreferrer"
                  className="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-center text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all shadow-sm cursor-pointer"
                  title="WhatsApp"
                >
          ...
          <p className="text-slate-400 dark:text-slate-500 text-[10px]">© 2026 All Rights Reserved.</p>
        </div>
      </footer>
              ...
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="font-bold">
                    {lang === 'en' 
                      ? 'SQL Database Sync Warning (Offline Fallback)' 
                      : 'SQL ডাটাবেস সিঙ্ক সতর্কতা (অফলাইন ব্যাকআপ)'}
                  </p>
                  <p className="text-xs text-amber-700 dark:text-amber-400 mt-1">
                    {lang === 'en'
                      ? `The backend SQL database connection temporarily failed. Don't worry! Your changes are being saved locally in your browser session and will sync automatically when the SQL backend is available.`
                      : `ব্যাকএন্ড SQL ডাটাবেস কানেকশন সাময়িকভাবে বিচ্ছিন্ন। চিন্তার কারণ নেই! আপনার পরিবর্তনগুলো ব্রাউজারে সেভ করা হচ্ছে এবং ব্যাকএন্ড সক্রিয় হলে স্বয়ংক্রিয়ভাবে সিঙ্ক হবে।`}
                  </p>
                </div>
              </div>
            </div>
